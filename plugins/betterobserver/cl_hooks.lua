...
function PLUGIN:PlayerNoClip(client, state)
	if (!hook.Run("CanPlayerEnterObserver", client) and state) then
		return false
	end
end
...
function PLUGIN:HUDPaint()
	...
	if (drawESP == nil) then
		drawESP = ix.option.Get("observerESP", true) and CAMI.PlayerHasAccess(client, "Helix - Observer ESP", nil)
	end

	if (drawESP) then
		local bObserver = client:GetMoveType() == MOVETYPE_NOCLIP and !client:InVehicle()
		local scrW, scrH = ScrW(), ScrH()
		local marginX, marginY = scrH * .1, scrH * .1

		if (!bObserver and ix.option.Get("observerESPOnlyInNoclip")) then return end
		self:DrawPlayerESP(client, scrW, scrH, bObserver)

		if (!bObserver) then return end
