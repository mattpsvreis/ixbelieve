...
function PLUGIN:DrawPlayerESP(client, scrW, scrH, drawRender)
    local pos = client:EyePos()
    local marginX, marginY = scrW * .05, scrH * .05
    self.traceFilter[1] = client

    local names = {}
    cam.Start3D()
        local lookDir = client:GetEyeTrace()
        local targets = hook.Run("GetAdminESPTargets") or player.GetAll()
        for _, v in ipairs(targets) do
            if (v == client or !v:GetCharacter() or
                (lookDir.Normal:Dot((v:GetPos() - pos):GetNormal()) < 0.65 and !ix.option.Get("thirdpersonEnabled"))) then
                continue
            end
    ...
    for _, info in ipairs(names) do
        ...
        if (ix.option.Get("thirdpersonEnabled") and (!eyePos.visible or eyePos.x < ScrW() * -0.1 or eyePos.y > ScrW() * 1.1)) then continue end
        ...
        if (ix.option.Get("playerInfoESP") and drawRender) then
