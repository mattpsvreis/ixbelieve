...
--ix.option.Add("HelixCrosshair", ix.type.bool, true, {
--	category = "appearance"
--})
...
if (CLIENT) then
	...
	function PLUGIN:PostDrawHUD()
		...
		if (ix.option.Get("HelixCrosshair", true) == false) then
			return
		end
...
function PLUGIN:HUDShouldDraw(element)
	if element == "CHudCrosshair" && !ix.option.Get("HelixCrosshair", false) then
		return true
	end
end
