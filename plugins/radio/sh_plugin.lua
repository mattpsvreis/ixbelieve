...
PLUGIN.author = "Gr4Ss & Dark"
PLUGIN.description = "Implements a better functional radio system. Based on my CW Radio Redux plugin. Credits to SleepyMode for the initial port (which this is very loosely based on). Edited for Halo: Believe by Dark."
...
ix.option.Add("radioYellNotifications", ix.type.array, "sl2ic", {
	category = "general",
	populate = function()
		return {all = "all", sl = "squad leads", sl2ic = "squad leads & 2IC's", never ="never"}
	end
})
...
ix.lang.AddTable("english", {
	...
	optRadioYellNotifications = "Radio Yell Notifications",
	optdRadioYellNotifications = "Get center-screen notifications when the specified people use radio yell."
})
...
do
	...
	if (CLIENT) then
		function CLASS:OnChatAdd(speaker, text, anonymous, data)
			...
			if (data.radioType == ix.radio.types.yell or data.radioType == ix.radio.types.scream) then
				local setting = ix.option.Get("radioYellNotifications")

				if (setting == "all") then
					ix.util.ScreenNotify(rText, color)
				elseif (setting == "sl" or setting == "sl2ic") then
					local mySquad = string.lower(LocalPlayer():GetNetVar("iffSquadName", ""))
					local sameSquad = mySquad != "" and string.lower(speaker:GetNetVar("iffSquadName", "")) == mySquad

					local targetFT = string.lower(speaker:GetNetVar("iffFireTeam", ""))
					local sameFT = targetFT == "" or targetFT == string.lower(LocalPlayer():GetNetVar("iffFireTeam", ""))
					local role = string.lower(speaker:GetNetVar("iffRole", ""))
					if (sameSquad and sameFT and (role == "actual" or (role == "2ic" and setting == "sl2ic"))) then
						ix.util.ScreenNotify(rText, color)
					end
				end
			end
	...
	if (CLIENT) then
		function CLASS:OnChatAdd(speaker, text, anonymous, data)
			...
			if (!data.stationary) then
				...
				if (data.radioType != ix.radio.types.whisper) then
				...
				if (speaker == LocalPlayer() or data.radioType != ix.radio.types.whisper) then
