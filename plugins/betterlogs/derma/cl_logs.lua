...
local function PaintFrames(self, w, h, bCustom)
	...
	if bCustom then
		...
		surface.DrawTexturedRectUV(0, 0, w, h, 0, 0, w / ScreenScaleMin(414 / 3), h / ScreenScaleMin(677 / 3))
	end
...
function PANEL:Init()
	...
	local titlePushDown = ScreenScaleMin(30 / 3)
	local topPushDown = ScreenScaleMin(400 / 3)
	local scale780 = ScreenScaleMin(780 / 3)
	local scale120 = ScreenScaleMin(120 / 3)
	...
	self.contentFrame.Paint = function(panel, w, h)
		PaintFrames(panel, w, h, false)
	end

	self:SetTall(scale120 + scale780 + titlePushDown)
	self:Center()
	self.columnWidth = ScreenScaleMin(150 / 3)
...
function PANEL:CreateTitleText()
	...
	--logsTitleIcon:SetSize(ScreenScaleMin(23 / 3), ScreenScaleMin(17 / 3))
	...
	self.logsTitle:SetPos(ScreenScaleMin(33 / 3), ScreenScaleMin(17 / 3) * 0.5 - self.logsTitle:GetTall() * 0.5)
end
...
function PANEL:Rebuild()
	...
	self.logs:SetHeaderHeight(ScreenScaleMin(16 / 3))
	self.logs:SetDataHeight(ScreenScaleMin(17 / 3))
	...
	for _, v in pairs(self.logs:GetChildren()) do
		...
		for _, v2 in pairs(v:GetChildren()) do
			...
			if (text == "Steam ID" or text == "Time") then
				v:SetFixedWidth(self.columnWidth)
			elseif (text == "ID") then
				v:SetFixedWidth(self.columnWidth * 2 / 3)
			end
		end
	end

	self.logs.OnRowSelected = function(list, lineId, line)
		if (!IsValid(self.logPreview)) then return end

		local record = line.record
		self.logPreview:SetValue(record.text)
	end
...
function PANEL:CreateLeftSideButtons()
  ...
  self.beforeTextEntry.OnEnter = search
	...
	self.descCheckBox.PerformLayout = function(panel)
		...
		panel.Button:SetSize(ScreenScaleMin(15 / 3), ScreenScaleMin(15 / 3))
		panel.Button:SetPos(x, math.floor((panel:GetTall() - panel.Button:GetTall()) / 2))

		panel.Label:SizeToContents()
		panel.Label:SetPos(x + panel.Button:GetWide() + ScreenScaleMin(9 / 3), 0)
	end
  ...
  if (!table.IsEmpty(self.requestedLogTypes)) then
	...
	self.logTypeCombo:SetTall(ScreenScaleMin(32 / 3))
	...
	self.logTypeCombo:AddChoice("all")
  ...
  searchButton:Dock(BOTTOM)
	...
	searchButton:SetTall(ScreenScaleMin(32 / 3))
...
function PANEL:CreatePagination()
	...
	paginationBg:SetTall(ScreenScaleMin(32 / 3))
	...
	self.firstPage:SetWide(self.firstPage:GetWide() + ScreenScaleMin(20 / 3))
...
function PANEL:CreateLogPreview()
	local logPreview = self.rightSide:Add("DTextEntry")
	self.logPreview = logPreview
	logPreview:Dock(BOTTOM)
	logPreview:DockMargin(4, 16, 4, 4)
	logPreview:SetTall(ScreenScaleMin(64 / 3))

	logPreview:SetMultiline(true)
	logPreview:SetEditable(false)
	logPreview:SetFont("MenuFontNoClamp")
	logPreview:SetTextColor(color_white)
	logPreview:SetPaintBackground(false)
end
...
function PANEL:CreateTextEntry(parent, bDockTop, height, bMultiline, bScrollbar, bEnter, placeholderText)
  parent:SetPlaceholderText(placeholderText)
	parent:Dock(TOP)
	parent:SetTall(ScreenScaleMin(height / 3))
