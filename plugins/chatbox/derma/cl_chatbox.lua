...
function PANEL:Init()
	...
	self.buttons.OnMousePressed = function (pnl, code) self.chatbox:OnMousePressed(code) end -- ix.util.Bind(self, self.OnMousePressed) -- we want mouse events to fall through
	self.buttons.OnMouseReleased = function (pnl, code) self.chatbox:OnMouseReleased(code) end
...
function PANEL:AddTab(id, filter)
	...
	panel.chatbox = self.chatbox

	button.DoRightClick = function(this)
		self.chatbox:OnTabRightClick(this, panel, panel:GetID())
	end
...
function PANEL:RemoveTab(id)
	...
	if (table.IsEmpty(self.tabs)) then
		...
		if not parent.name then
			...
			parent:Remove()
			PLUGIN.additionalPanels[parent.name] = nil
		end
...
function PANEL:PerformLayoutInternal()
	local bar = self:GetVBar()
	local bScroll = !self.chatbox:GetActive() or bar.Scroll == bar.CanvasSize -- only scroll when we're not at the bottom/inactive
...
function PANEL:Init()
	...
	self.entry.chatbox = self

	self.prefix = entryPanel:Add("ixChatboxPrefix")
	self.prefix.chatbox = self
	...
	self.autocomplete.chatbox = self
	...
	self:Receiver("ixChatboxTab", function (pnl, tbl, dropped, index, x, y)
	end, {})
end
...
function PANEL:SetupTabs(tabs, ignore)
	if (!tabs or table.IsEmpty(tabs) and not ignore) then
...
function PANEL:OnMousePressed(key)
	if (key == MOUSE_RIGHT) then
		local menu = DermaMenu()
			menu:AddOption("New chat box...", function()
				...
				panel.OnTabCreated = function (p, id, filter, value)
					local chat = PLUGIN:CreateExtraChat(id)
					chat:OnTabCreated(id, filter, value)

					PLUGIN:SaveTabs(nil, true)
					PLUGIN:SavePosition(self)
				end

				ix.util.Bind(self, self.OnTabCreated)
			end)
